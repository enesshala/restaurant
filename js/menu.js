...
function renderCategories(categories) {
    ...
    Object.keys(categories).forEach((categoryName, index) => {
        ...
        categoryElement.innerHTML = `
          <img src="img/pasta.svg" alt="${categoryName}" width="200px"/>
          <div class="menu-category-info">
              <h3 class="menu-category-title">${categoryName}</h3>
              <p class="text-gray">${categories[categoryName].length} Menu items</p>
          </div>
        `;
    ...
    items.forEach((item) => {
        ...
        menuItemElement.innerHTML = `
            <div class="menu-item-header">
                <img src="${item.image}" alt="${item.name}" class="menu-item-img" width="200px"/>
                <div class="menu-item-title">
                    <h3>${item.name}</h3>
                    <p class="text-gray menu-item-desc">
                        ${item.ingradients ? item.ingradients : "No ingradients description."}
                    </p>
                </div>
            </div>
...
function renderInvoice() {
    ...
    cart.forEach((item, index) => {
        ...
        invoiceItemElement.innerHTML = `
            <img src="${item.image}" alt="${item.name}" width="200px" class="invoice-item-img" />
    ...
    paymentSummaryContainer.innerHTML = `
        ...
        <button id="pay-button" class="pay-button">Pay $${totalPayment.toFixed(2)}</button>
    `;
